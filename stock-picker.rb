def stock_picker(arr)
  ...
  (0..(arr.length - 2 )).each do |buy_date|
    ...
    ((buy_date + 1)..arr.length - 1).each do |sell_date|
      
      #check the profit for this pair of days
      price_difference = arr[sell_date] - arr[buy_date]
  ...
  puts "day #{best_buy}: buy at #{arr[best_buy]}"
  puts "day #{best_sell}: sell at #{arr[best_sell]}"
